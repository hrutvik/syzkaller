...
static int setup_loop_device(long unsigned size, long unsigned compressed_size, unsigned char* data, const char* loopname, int* memfd_p, int* loopfd_p)
{
	...
	dest = fdopen(memfd, "w");
	if (errno) {
		debug("setup_loop_device: could not create output stream: %d\n", errno);
		goto error_close_memfd;
	}

	err = decompress(data, compressed_size, dest);
	if (err) {
...
// syz_read_part_table(size intptr, size_compressed intptr, img ptr[in, compressed])
