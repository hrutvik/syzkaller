...
func TestGenericHeatmap(t *testing.T) {
	...
	testData := []struct {
		...
		{
			// Test all constant bytes, i.e. no regions and uniform selection.
			[]byte(
...
func checkIndex(index, maxIndex int, regions []region) bool {
	...
	if len(regions) == 0 {
		// Test that index is chosen uniformly up to maxIndex.
		return true
	}
...
func (hm *GenericHeatmap) debugPrint(t *testing.T, data []byte, regions []region) {
	...
	// Print heatmap.
	t.Logf("generic heatmap (total segment length %d, total length %d)", hm.length, hm.rawLength)
