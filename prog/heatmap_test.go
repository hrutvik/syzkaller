	...
	for i := 0; i < iters; i++ {
		...
		for j := 0; j < tries; j++ {
			...
			if !checkIndex(index, len(data), regions) {
				...
				debugPrintGenericHeatmap(data, regions, hm)
			}
		}
	}
}
...
func createData(r *rand.Rand) ([]byte, []Region) {
	// Initialise slice up to 1 MB in length.
	len := r.Intn(1<<20) + 1
	data := make([]byte, len)
	...
	// Randomly select "interesting" regions. We don't care about overlapping regions.
	numRegions := r.Intn(10)
	var regions []Region
	for i := 0; i < numRegions; i++ {
		start := r.Intn(len)
		length := r.Intn((len/100)+1) + (granularity / 5) // Ensure non-empty.
		if start+length > len {
			continue
		}
...
func debugPrintGenericHeatmap(data []byte, regions []Region, hm *GenericHeatmap) {
	// Print data.
