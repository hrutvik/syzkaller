	...
	prog.TestDeserializeHelper(t, targets.Linux, targets.AMD64, nil, []prog.DeserializeTest{
		{
			...
			In: `syz_mount_image$bfs(&(0x7f0000000000)='bfs\x00', &(0x7f0000000100)='./file0\x00', 0xdeadbeef, 0x9, 0x0, &(0x7f00000100a0)={[], [], 0x0}, 0x0, &(0x7f0000000200)='$c3l6a2FsbGVy')`,
		},
		{
			// Invalid compressed size.
			In: `syz_mount_image$bfs(&(0x7f0000000000)='bfs\x00', &(0x7f0000000100)='./file0\x00', 0xdeadbeef, 0xdeadbeef, 0x0, &(0x7f00000100a0)={[], [], 0x0}, 0x0, &(0x7f0000000200)='$c3l6a2FsbGVy')`,
			// It should be able to fix up the size.
			Out: `syz_mount_image$bfs(&(0x7f0000000000)='bfs\x00', &(0x7f0000000100)='./file0\x00', 0xdeadbeef, 0x9, 0x0, &(0x7f00000100a0)={[], [], 0x0}, 0x0, &(0x7f0000000200)='$c3l6a2FsbGVy')`,
		},
	})
}
...
func TestExtractSyzMountImage(t *testing.T) {
	...
	for _, file := range files {
		...
		for _, asset := range p.ExtractAssets() {
			...
			var compressedTestResult []byte
			if asset.Reader != nil {
				var err error
				compressedTestResult, err = io.ReadAll(asset.Reader)
				if err != nil {
					t.Fatal(err)
				}
			}
			testResult, err := prog.Decompress(compressedTestResult)
			if err != nil {
				t.Fatalf("could not decompress image extracted from %s: %v", file, err)
			}
