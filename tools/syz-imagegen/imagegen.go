...
func writeImage(image *Image, data []byte, chdir bool) ([]byte, error) {
	...
	fmt.Fprintf(buf, "# %v\n\n", image)
	compressedData, err := prog.Compress(data)
	if err != nil {
		return nil, fmt.Errorf("failed to compress: %v", err)
	}
	b64Data, err := prog.EncodeB64(compressedData)
	if err != nil {
		return nil, fmt.Errorf("failed to encode in Base64: %v", err)
	}

	addrFsOptions := 0x7f0000010000 + len(compressedData)
	chdirAsInt := 0
	if chdir {
		chdirAsInt = 1
	}
	fmt.Fprintf(buf, `syz_mount_image$%v(&(0x7f0000000000)='%v\x00', &(0x7f0000000100)='./file0\x00',`+
		` 0x%x, 0x%x, 0x0, &(0x%x), 0x%x, &(0x7f0000000200)='$`,
		image.fs.Name, image.fs.Name, len(data), len(compressedData), addrFsOptions, chdirAsInt)
	buf.Write(b64Data)
	fmt.Fprintf(buf, "')\n")
...
//	fmt.Fprintf(buf, `syz_read_part_table(0x%x, 0x%x, &(0x7f0000000200)='#`,
